...
/*
	unsigned long lora_Noloop(bool SENDER, String Msg)
		Remplaza a la función lora_Loop cuando se desea utilizar espeficificamente un modo de 
	comunicación (Transmisor/Receptor).
		Toma como parametros el estado emisor, recepctor (mediante la variable SENDER):
...
String lora_Noloop(bool SENDER, String Msg) {
  ...
  if (SENDER) {
    ...
    // Comienzo de la tranmisión
    LoRa.beginPacket();
    LoRa.print(Msg);  //Mensaje a TRANSMITIR
    LoRa.endPacket();
    Serial.println("Mensaje Enviado");
  ...
  /******** MODO RECEPCIÓN *******/
    ...
    digitalWrite(LEDRX, HIGH);  //Enciende el led recepción
    ...
    if(strlen(mensajeRecibido) > 2)
      return mensajeRecibido;   
    else (t0 >= stoi(Msg)) {     //Si no se recibio nada, espera un TimeOut y sale
      Serial.println("Timeout ");
      return "-1";
    }
  }
